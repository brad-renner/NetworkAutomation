# Take user input of WLAN/SSID selection & disable the WLAN administratively or disable the broadcasting of the WLAN's SSID.
...
param(
    [Parameter(Mandatory=$True, 
        HelpMessage = 'Please chose the WLAN/SSID to be disabled/disabled broadcast)')]
    [ValidateSet("Internal", "Scanner", "POS", "Guest")]
    [string]$wlan,
    [Parameter(Mandatory=$True, 
        HelpMessage = 'Please chose which WLC the WLAN/SSID should be disabled on from the following options: ABCWLC9800-1, ABCWLC9800-2, ABCWLC9800-3, ABCNWLC9800, ABCLABWLC9800L-1')]
    [ValidateSet("ABCWLC9800-1","ABCWLC9800-2", "ABCWLC9800-3", "ABCNWLC9800", "ABCLABWLC9800L-1")]
    [string]$WLC,
    [switch]$disableBroadcastSSID,
    [switch]$dryrun
)
